...
#!/bin/bash
# 1. Prepare a writable cacerts file in our scratch space
WRITABLE_CACERTS="/java-security/cacerts"
ORIGINAL_CACERTS="${JAVA_HOME}/lib/security/cacerts"

echo "Copying system cacerts to writable scratch space..."
cp "${ORIGINAL_CACERTS}" "${WRITABLE_CACERTS}"

# 2. Update the Java environment variable so the app uses our new file
